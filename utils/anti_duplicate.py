...
class AntiDuplicate:
    ...
    async def is_duplicate_signal(self, symbol: str, signal_type: str,
                           entry_price: float, timeframe: str = '1h') -> bool:
        """Kiểm tra xem tín hiệu có trùng không"""
        try:
            # Kiểm tra cooldown
            if await self._check_cooldown(symbol, signal_type):
                logger.info(f"Signal blocked by cooldown: {signal_type} {symbol}")
                return True

            # Kiểm tra tín hiệu tương tự gần đây
            if await self._check_similar_signals(symbol, signal_type, entry_price):
    ...
    async def _check_cooldown(self, symbol: str, signal_type: str) -> bool:
        """Kiểm tra cooldown period"""
        try:
            last_signal = await db.get_last_signal_time_async(symbol, signal_type) if hasattr(db, 'get_last_signal_time_async') else db.get_last_signal_time(symbol, signal_type)
    ...
    async def _check_similar_signals(self, symbol: str, signal_type: str,
                             entry_price: float) -> bool:
        """Kiểm tra tín hiệu tương tự trong thời gian gần"""
        try:
            # Lấy các tín hiệu gần đây (use async version)
            recent_signals = await db.get_recent_signals_async(symbol=symbol, limit=5) if hasattr(db, 'get_recent_signals_async') else db.get_recent_signals(symbol=symbol, limit=5)
